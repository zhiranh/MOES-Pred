...
class QM9(QM9_geometric):
    ...
    root：数据集存储的根目录。
    transform：一个可调用的函数/变换，用于处理 Data 对象。
    dataset_arg：要训练的目标属性。
    add_radius_edge：是否添加基于半径的边（默认为 False）。
    ...
    def __getitem__(self, idx: Union[int, np.integer, IndexType]) -> Union['Dataset', Data]:
        """
        Retrieves and processes a data1 item at the specified index.

        Args:
            idx (int): Index of the data1 item to retrieve.

        Returns:
            Data: Processed data1 item.
        """
