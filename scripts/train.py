...
def get_args():
    ...
    parser.add_argument('--num-workers', type=int, default=4, help='Number of workers for data1 prefetch')
    ...
    parser.add_argument('--dataset-root', default='data1', type=str, help='Data storage directory (not used if dataset is "CG")')
...
def main():
    ...
    #使用 prepare_data() 和 setup("fit") 方法准备数据并设置训练阶段。
    data.prepare_data()
    data.setup("fit")
    ...
    if args.prior_model:
        ...
        args.prior_args = prior.get_init_args()  #1111111111111

    # initialize lightning module 用于定义模型的训练逻辑。
    model = LNNP(args, prior_model=prior, mean=data.mean, std=data.std)

    #初始化模型检查点回调和早停回调，用于保存最佳模型和提前停止训练。
    ...
    #连续 N 个 epoch val_loss 不降就自动结束训练，防止过拟合+浪费时间。
    early_stopping = EarlyStopping("val_loss", patience=args.early_stopping_patience)
    #初始化 TensorBoard、CSV 和 Weights & Biases（Wandb）日志记录器，用于记录训练过程中的指标。
    ...
    #初始化 PyTorch Lightning 训练器，配置训练过程中的各种参数
    trainer = pl.Trainer(
