...
# code from equibind:检查一个环中的所有键是否都是芳香键。
...
#train  test
#LoadFromFile 让你用一行命令 + 一个 YAML 文件就能替代冗长的命令行参数，自动校验合法性并支持命令行覆盖，是实验配置管理的“轻量级瑞士军刀”。
class LoadFromFile(argparse.Action):
    """
    Custom argparse action to load configuration from a YAML file.
    LoadFromFile 是一个自定义的 argparse.Action，用于从 YAML 文件中加载配置。它读取 YAML 文件的内容，
    并将文件中的键值对更新到 argparse 的命名空间中。它还验证 YAML 文件中的所有键是否都是有效的命令行参数，如果不是，则抛出错误
    Usage:
...
#train test
#LoadFromCheckpoint 让 --load_model 不只是指定权重文件，而是一次性把训练时的完整超参“复活”到当前命令行，实现真正的断点续训 / 推理复现。
class LoadFromCheckpoint(argparse.Action):
    ...
    """
    #自定义的 argparse.Action 类，用于从文件中加载参数。
    # parser.add_argument('--file', type=open, action=LoadFromFile)
    def __call__(self, parser, namespace, values, option_string=None):
...
#保存 argparse 参数到 YAML 文件的函数 save_argparse
...
#将文本形式的数字转换为适当的数值类型（int 或 float）
def number(text):
    """
...
#这个类用于将多个图合并成一个大的图，并通过 batch 属性来区分每个节点属于哪个图。

class BatchMasking(Data):
    r"""A plain old python object modeling a batch of graphs as one big
    (dicconnected) graph. With :class:`torch_geometric.data1.Data` being the
    ...
    #从一个包含 torch_geometric.data1.Data 对象的列表中构造一个 BatchMasking 对象。
    @staticmethod
    def from_data_list(data_list):
        r"""Constructs a batch object from a python list holding
        :class:`torch_geometric.data1.Data` objects.
        ...
        for i, data in enumerate(data_list):
            ...
            # cumsum_edge += data1.edge_index.shape[1]
    ...
    def cumsum(self, key, item):
        ...
        .. note::
            ...
            if the batch concatenation process is corrupted for a specific data1
            attribute.
        """
...
#data1
#提供了一个方法 batch_stack 用于处理和堆叠张量
class DataLoaderMasking(torch.utils.data.DataLoader):
    r"""Data loader which merges data1 objects from a
    :class:`torch_geometric.data1.dataset` to a mini-batch.
    Args:
        dataset (Dataset): The dataset from which to load the data1.
        batch_size (int, optional): How may samples per batch to load.
            (default: :obj:`1`)
        shuffle (bool, optional): If set to :obj:`True`, the data1 will be
...
#将一个列表中的 PyTorch 张量堆叠成一个张量，并确保每个张量在每个轴上都被填充到最大张量的大小。
