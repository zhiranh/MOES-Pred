...
from torchmdnet.models import priors

from .moes_modules import BRICSMotifAggregation, InfluenceMatrix, EnergySentinel, MolecularPotential
...
def create_model(args, prior_model=None, mean=None, std=None):
    ...
    if args["model"] == "equivariant-transformer":
        ...
        # )
        representation_model = EGNN_finetune_last(
            in_node_nf=300, hidden_nf=args['hidden_nf'], in_edge_nf=4, act_fn=nn.SiLU(), n_layers=args['n_layers'],
            residual=True, attention=True, normalize=False, tanh=False, use_layer_norm=False
        )
    ...
    if args['output_model_noise'] is not None:
        if args['bond_length_scale']:
            ...
            output_model_noise = nn.ModuleList(
                [output_bond_noise, output_angle_noise, output_dihedral_noise, output_rotate_dihedral_noise])
    ...
    if args['mask_atom']:
        output_model_mask_atom = getattr(output_modules, "MaskHead")(
            args["embedding_dimension"], args["activation"], )
...
pcq_with_h = {'name': 'pcq',
              'atom_encoder': {'H': 0, 'He': 1, 'Be': 2, 'B': 3, 'C': 4, 'N': 5, 'O': 6, 'F': 7, 'Mg': 8, 'Si': 9,
                               'P': 10, 'S': 11, 'Cl': 12, 'Ar': 13, 'Ca': 14, 'Ti': 15, 'Zn': 16, 'Ga': 17, 'Ge': 18,
                               'As': 19, 'Se': 20, 'Br': 21},
              'atomic_nb': [1, 2, 4, 5, 6, 7, 8, 9, 12, 14, 15, 16, 17, 18, 20, 22, 30, 31, 32, 33, 34, 35],
              'atom_decoder': ['H', 'He', 'Be', 'B', 'C', 'N', 'O', 'F', 'Mg', 'Si', 'P', 'S', 'Cl', 'Ar', 'Ca', 'Ti',
                               'Zn', 'Ga', 'Ge', 'As', 'Se', 'Br'],
              'max_n_nodes': 53,
              'n_nodes': {2: 77, 3: 62, 4: 162, 5: 439, 6: 721, 7: 1154, 8: 1879, 9: 2758, 10: 4419, 11: 6189, 12: 9283,
                          13: 12620, 14: 18275, 15: 24340, 16: 31938, 17: 40477, 18: 51301, 19: 62211, 20: 74714,
                          21: 87453, 22: 103873, 23: 121040, 24: 135340, 25: 148497, 26: 165882, 27: 177003, 28: 185152,
                          29: 187492, 30: 204544, 31: 183114, 32: 183603, 33: 177381, 34: 174403, 35: 147153,
                          36: 129541, 37: 113794, 38: 99679, 39: 79646, 40: 59481, 41: 46282, 42: 36100, 43: 26546,
                          44: 17533, 45: 15672, 46: 13709, 47: 7774, 48: 1256, 49: 5445, 50: 955, 51: 118, 52: 1,
                          53: 125},
              'atom_types': {0: 51915415, 1: 5, 2: 2, 3: 17730, 4: 35554802, 5: 5667122, 6: 4981302, 7: 561570, 8: 2,
                             9: 33336, 10: 40407, 11: 506659, 12: 310138, 13: 3, 14: 2, 15: 4, 16: 4, 17: 4, 18: 369,
                             19: 299, 20: 1459, 21: 36399},
              'colors_dic': ['#FFFFFF99', 'C2', 'C7', 'C0', 'C3', 'C1', 'C5', 'C6', 'C4', 'C8', 'C9', 'C10', 'C11',
                             'C12', 'C13', 'C14', 'C15', 'C16', 'C17', 'C18', 'C19', 'C20'],
              'radius_dic': [0.3, 0.6, 0.6, 0.6, 0.6, 0.6, 0.6, 0.6, 0.6, 0.6, 0.6, 0.6, 0.6, 0.6, 0.6, 0.6, 0.6, 0.6,
                             0.6, 0.6, 0.6, 0.6], 'with_h': True}
        ...
        for k in current_model_dict:
            if k in new_state_dict:
                if current_model_dict[k].size() == new_state_dict[k].size():
                    ...
                    new_state_dict2[k][pcq_with_h['atomic_nb']] = new_state_dict[k].T[:-1] + new_state_dict[
                        k.split('.weight')[0] + '.bias']
...
class TorchMD_Net(nn.Module):
    def __init__(
        ...
        self.motif_aggregation = BRICSMotifAggregation(
            hidden_channels=representation_model.hidden_channels if hasattr(representation_model, 'hidden_channels') else 128
        )
        ...
        if self.bond_length_scale > 0 and not self.no_target_mean:
            ...
            hidden_channels = self.representation_model.hidden_channels if hasattr(self.representation_model, 'hidden_channels') else 128
    ...
    def forward(self, z, pos, batch: Optional[torch.Tensor] = None, batch_org=None, egnn_dict=None,
                radius_edge_index=None, pos_gt=None, motif_masks=None):
        """
        ...
        Args:
            ...
            pos_gt (torch.Tensor, optional): Ground truth atomic positions for energy sentinel
            motif_masks (torch.Tensor, optional): BRICS motif masks for motif aggregation
        Returns:
        ...
        if egnn_dict is not None:
            pred, noise_pred = self.representation_model(h=z, x=pos, edges=egnn_dict['edges'],
                                                         edge_attr=egnn_dict['edge_attr'],
                                                         node_mask=egnn_dict['node_mask'], n_nodes=egnn_dict['n_nodes'],
        ...
        sk_score = torch.tensor(0.0, device=z.device)
        ens_loss = torch.tensor(0.0, device=z.device)
        if motif_masks is not None:

            motif_x = self.motif_aggregation(x, motif_masks, batch)

            atomic_weights = self.influence_matrix(x, motif_x)

            if pos_gt is not None:
                atom_potential = self.potential_energy(pos)
                weighted_energy = (atomic_weights * atom_potential).sum()
                score, rec_loss, aux_loss = self.energy_sentinel.score_noise_scheme(
                    pred_pos=pos, true_pos=pos_gt, atomic_weights=atomic_weights, potential_energy=weighted_energy
                )
                self.energy_output = (score, rec_loss, aux_loss, weighted_energy)
                sk_score = score
        ...
        if self.bond_length_scale > 0:
            ...
            # dihedral_x = self.dihedral_jk_proj(torch.cat([dihedral_j_x, dihedral_k_x], axis=1))
            rotate_dihedral_x = torch.cat(
                [rotate_dihedral_i_x, rotate_dihedral_j_x, rotate_dihedral_k_x, rotate_dihedral_l_x], axis=1)
        ...
        if self.output_model_noise is not None:
            if nv is not None:
                ...
                if self.bond_length_scale > 0:
                    ...
                    angle_noise_pred = self.output_model_noise[1].pre_reduce(angle_x, angle_v, z, pos, batch).mean(
                        axis=1)
                    dihedral_noise_pred = self.output_model_noise[2].pre_reduce(dihedral_x, dihedral_v, z, pos,
                                                                                batch).mean(axis=1)
                    rotate_dihedral_noise_pred = self.output_model_noise[3].pre_reduce(rotate_dihedral_x,
                                                                                       rotate_dihedral_v, z, pos,
                                                                                       batch).mean(axis=1)
        ...
        # scale by data1 standard deviation
        ...
        return out, noise_pred, dy, mask_logits, sk_score, ens_loss
