...
#生成归一化的球贝塞尔函数基。一个包含每个阶数和零点的符号表达式的列表。
...
#通过使用包络函数和正弦径向基函数，可以有效地处理距离信息，避免数值不稳定问题。 将输入的距离转换为嵌入向量
...
# angle_emb 类是一个 PyTorch 模块，用于计算角度嵌入。它结合了球谐函数（spherical harmonics）和贝塞尔函数（Bessel functions），
# 将输入的距离和角度转换为嵌入向量
...
#扭转嵌入（torsional embeddings）。它结合了球谐函数（spherical harmonics）和贝塞尔函数（Bessel functions），
# 将输入的距离、角度（theta）和方位角（phi）转换为嵌入向量。
...
#xyz_to_dat 函数用于从几何信息（节点的3D坐标）计算图中的距离、角度和扭转角。
# 这个函数在处理分子图和其他具有几何信息的图结构数据时非常有用。以下是对 xyz_to_dat 函数的详细解释：
