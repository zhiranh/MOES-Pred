...
#快速可视化特定类型的基函数。这个函数可以帮助检查基函数在非默认的上下截止距离之间的覆盖情况
...
#用于在图神经网络中进行邻居嵌入。它结合了原子的嵌入和距离信息，通过消息传递更新节点特征
...
#ShiftedSoftplus 是一个激活函数，它是 Softplus 激活函数的一个变体，通过常数偏移确保在零输入时输出为零
...
#用于实现余弦截止函数。这个函数用于平滑地截断原子间的距离，确保在接近截止距离时，原子间的相互作用平滑地过渡到零。
...
#用于计算原子间的距离和对应的向量。它基于原子的位置数据，可以生成图的边索引、边权重和边向量 针对3D数据的
class Distance(nn.Module):
    """
    Module for computing interatomic distances and corresponding vectors based on position data1.
...
#用于实现门控等变块（Gated Equivariant Block）。这个模块在处理图神经网络中的向量和标量特征时非常有用
# 通过更新网络更新标量和向量特征。
