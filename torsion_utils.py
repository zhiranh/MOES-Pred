...
#提取二面角：识别分子中所有可调整的二面角。get_torsions
#设置二面角：根据需要调整特定的二面角。SetDihedral
#获取二面角：检查当前的二面角值，确保调整正确。GetDihedral
#批量调整：对多个二面角进行批量调整，以探索分子的构象空间。apply_changes


#从分子列表中提取所有可能的二面角（torsion angles）。
def get_torsions(mol_list):
    ...
    torsionList = []
    dihedralList = []
    #mol_list 是一个 RDKit 分子对象的列表。
    #将所有有效的二面角索引存储在 torsionList 中。
    for m in mol_list:
        torsionSmarts = '[!$(*#*)&!D1]-&!@[!$(*#*)&!D1]'
...
def add_equi_noise_new(opt_mol, bond_var=0.058, angle_var=0.129, torsion_var_r=1, torsion_var=0.18, coord_var=0.04, add_ring_noise=False, add_noise_type=0, mol_param=None, ky=0):
    ...
    if add_noise_type == 2:
        ...
        # dihedral_label_lst = [] # [i, j, k, l, delta_angle]遍历分子中的每个键，获取键的两个端点原子的索引 j_idx 和 k_idx。
        rotate_dihedral_label_lst = []
        for bond in mol.GetBonds():
...
def add_equi_noise_nnew(opt_mol, add_bond:bool,add_angel:bool, add_torsion:bool, bond_var, angle_var, torsion_var, coord_var, torsion_var_r=1,
                       add_ring_noise=False, add_noise_type=0, mol_param=None, ky=0):
    """
        ...
        if add_torsion:
            ...
            for bond in mol.GetBonds():
                ...
                if mol_param is not None:
                    try:
                        torsion_para = mol_param[0]["ProperTorsions"][(i_idx, j_idx, k_idx, l_idx)].k[0]._magnitude
                        torsion_period = mol_param[0]["ProperTorsions"][(i_idx, j_idx, k_idx, l_idx)].periodicity[
                            0]  # for torsion i-j-k-l
    ...
    bond_len_lst, angle_lst, dihedral_lst = get_info_by_gem_idx(opt_mol, edge_idx, angle_idx, dihedral_idx)
    bond_len_lst_noise, angle_lst_noise, dihedral_lst_noise = get_info_by_gem_idx(mol, edge_idx, angle_idx,
                                                                                  dihedral_idx)
    # noise difference, has filter nan
    bond_label_lst, angle_label_lst, dihedral_label_lst = concat_idx_label(edge_idx, angle_idx, dihedral_idx,
                                                                           bond_len_lst_noise - bond_len_lst,
                                                                           angle_lst_noise - angle_lst,
                                                                           dihedral_lst_noise - dihedral_lst)
    # edge: [i,j, noise(i,j)]
    # angle: [i, j, k, noise(i,j,k)]
    # filter zero
    if angle_label_lst.size != 0:
    ...
    if mol_param is not None:
        ...
        for torsion_label in dihedral_label_lst:
            i_idx, j_idx, k_idx, l_idx = int(torsion_label[0]), int(torsion_label[1]), int(torsion_label[2]), int(
                torsion_label[3]),
