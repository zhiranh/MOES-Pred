...
def inner_smi2coords(smi, seed=42, mode='fast', remove_hs=False):
    ...
    #使用 Chem.MolFromSmiles 将 SMILES 字符串转换为 RDKit 分子对象。
    #使用 AllChem.AddHs 添加氢原子。
...
# parse smiles file from args 解析命令行参数，获取输入的 SMILES 文件和输出文件路径
import argparse
parser = argparse.ArgumentParser(description='user sample')
parser.add_argument('--smiles_file', type=str, default='tryout.lst', help='input smiles list file')
parser.add_argument('--output_file', type=str, default='tryout_coord.lst', help='output save result file')
